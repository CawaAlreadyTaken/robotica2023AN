...
public:
  ...
  DifferentialKinematic()
  {
    ...
    // current_position << -0.32, -0.78, -2.56, -1.63, -1.57,  3.49; // first configuration of homing position
  ...
  void setRobotsPosition(Matrix<double, 6, 1> joints)
  {
    ...
    rotation << rototranslation.row(0).head(3), rototranslation.row(1).head(3), rototranslation.row(2).head(3);
    this->R = rotation;

    this->ee_coords = rototranslation.col(3).head(3);

    cout << "[*] Set robot position on diffKin:" << endl;
    cout << "[*] Joints: " << this->current_position << endl;
    cout << "[*] Rotation: " << this->R << endl;
    cout << "[*] EE coords: " << this->ee_coords << endl;
  }
  ...
  Matrix<double, 6, 1> Qdot(Matrix<double, 6, 1> joint_states, Vector3d desired_pos, Vector3d desired_pos_vel, Vector3d desired_RPY, Vector3d desired_RPY_velocities, Matrix<double, 3, 3> Kq, Matrix<double, 3, 3> KRPY)
  {
    ...
    //cout << abs(min_eigenvalue) << endl;
    /*if (abs(min_eigenvalue) < 0.4)
      jac = dampedJacobian(jac, 0.01);*/
  ...
  double close_to_collisions(Vector3d pos1, Vector3d pos2, Matrix<double, 6, 1> joints)
  {
    ...
    Vector3d pos1_rcoords;
    pos1_rcoords = fromWorldToUr5(pos1);
    Vector3d pos2_rcoords;
    pos2_rcoords = fromWorldToUr5(pos2);
...
private:
  Vector3d fromRotToRPY(Matrix<double, 3, 3> R) // radianti
  {
    ...
    if (abs(R(0, 0) - R(0, 1)) < epsilon && abs(R(0, 0)) < epsilon)
  ...
  vector<Vector3d> get_joints_positions(Matrix<double, 6, 1> joints)
  {
    ...
    vector<Vector3d> res;
    res.push_back(fromUr5ToWorld((T10 * homogeneous_zero).head(3)));
    res.push_back(fromUr5ToWorld((T10 * T21 * homogeneous_zero).head(3)));
    res.push_back(fromUr5ToWorld((T10 * T21 * T32 * homogeneous_zero).head(3)));
    res.push_back(fromUr5ToWorld((T10 * T21 * T32 * T43 * homogeneous_zero).head(3)));
    res.push_back(fromUr5ToWorld((T10 * T21 * T32 * T43 * T54 * homogeneous_zero).head(3)));
    res.push_back(fromUr5ToWorld((T10 * T21 * T32 * T43 * T54 * T65 * homogeneous_zero).head(3)));
